...
def create_prot_esm_dgl_graph(pdb_name, data_dir):
    def get_graph(pdb_name, data_dir):
        ...
        # print("Graph created with {} nodes and {} edges".format(g_dgl.num_nodes(), g_dgl.num_edges()))
        return g_dgl, features_to_concat
    
    pdb_path = os.path.join(data_dir, str(pdb_name))
    # print(pdb_path)
...
# from your_module import create_prot_esm_dgl_graph  
dataset = "biosnap"  # biosnap

pdb_dir = f"../predata/{dataset}/pdb"                  
save_dir = f"../data/{dataset}/protein_150M_graphs_pkl"     
os.makedirs(save_dir, exist_ok=True)


for file in tqdm(os.listdir(pdb_dir), desc="Encoding PDBs"):
    if not file.endswith(".pdb"):
        continue

    pdb_id = os.path.splitext(file)[0]  
    pdb_path = os.path.join(pdb_dir, file)
    save_path = os.path.join(save_dir, f"{pdb_id}.pkl")

    if os.path.exists(save_path): 
        continue

    try:
        graph,features_to_concat = create_prot_esm_dgl_graph(pdb_id, pdb_dir)
        with open(save_path, "wb") as f:
            pickle.dump({"pdb_id": pdb_id, "graph": graph, "features_to_concat": features_to_concat}, f)
    except Exception as e:
        print(f"[Warning] Skipping {pdb_id}: {e}")
...
class DrugChemBertFeaturizer():
    def __init__(self):
        super().__init__()
        model_name = "../BioEncoder/ChemBERTa-77M-MLM"
        new_tokenizer = Tokenizer(
            WordLevel.from_file(
                '../BioEncoder/ChemBERTa-77M-MLM/vocab.json', 
                unk_token='[UNK]'
        ))
...
class DrugChemBertGNNFeaturizer():
    def __init__(self):
        super().__init__()
    ...
    def transform(self, x):
        ...
        bert_embedding = self.bert.transform(x)
        graph.ndata['bert'] = bert_embedding
        return graph, bert_embedding
import os
import pandas as pd
...
DATA = ["biosnap"] # ["human","celegans","DAVIS"，"biosnap"]
SPLIT = ["full_data"]
for dataset in DATA:
    for x in SPLIT:
        for split in ["train", "val", "test"]:
            
            src_path = f"../predata/{dataset}/{x}/{split}.csv"
            save_path = f"../data/{dataset}/{x}/{split}.csv"     
            save_dir = save_path.replace(f"{split}.csv", f"test_chembert_77M_MLM__graph_bert_embedding_{split}.pkl")

            os.makedirs(os.path.dirname(save_dir), exist_ok=True)

            df = pd.read_csv(src_path, sep=',')
            data = df[['DrugBank ID', 'SMILES']]

            
            featurizer = DrugChemBertGNNFeaturizer()

            
            records = []

            
            for idx, row in tqdm(data.iterrows(), total=len(data), desc="Featurizing SMILES"):
                drug_id = row['DrugBank ID']
                smiles = row['SMILES']

                try:
                    graph, bert_embedding = featurizer.transform(smiles)
                    records.append({
                        'drug_id': drug_id,
                        'graph': graph,
                    ...
                    print(f"[Warning] Skipping {drug_id}: {e}")
                    continue
